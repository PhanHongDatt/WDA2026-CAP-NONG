...
public class DataInitializer implements CommandLineRunner {
        ...
        public void run(String... args) {
                // Evict stale Redis caches on startup to prevent deserialization errors
                try {
                        var cache = cacheManager.getCache("seasonal_configs");
                        if (cache != null) {
                                cache.clear();
                                log.info("🧹 Cleared seasonal_configs cache");
                        }
                } catch (Exception e) {
                        log.warn("⚠️ Could not clear cache: {}", e.getMessage());
                }
                ...
                // ─── 1. Tài khoản ──────────────────────────────────────
                User admin = ensureUser("admin", "admin@capnong.shop", "0900000001", "Quản Trị Viên", Role.ADMIN,
                                encodedPassword);
                User buyer = ensureUser("buyer", "buyer@capnong.shop", "0900000002", "Nguyễn Văn Mua", Role.BUYER,
                                encodedPassword);
                User farmer1 = ensureUser("farmer", "farmer@capnong.shop", "0900000003", "Trần Văn Nông", Role.FARMER,
                                encodedPassword);
                User farmer2 = ensureUser("farmer2", "farmer2@capnong.shop", "0900000006", "Lê Thị Hoa", Role.FARMER,
                                encodedPassword);
                User htxMember = ensureUser("htx_member", "htxmember@capnong.shop", "0900000004", "Phạm Minh Đức",
                                Role.HTX_MEMBER, encodedPassword);
        ...
        private User ensureUser(String username, String email, String phone,
                ...
                if (existingUserOpt.isPresent()) {
                        ...
                        log.info("   ⏩ {} ({}) — đã tồn tại (Username: {}, Phone: {}, CurrentRole: {})", fullName, role,
                                        existing.getUsername(), existing.getPhone(), existing.getRole());

                        // DON'T forcibly reset the role — it may have been changed dynamically
                        // (e.g., FARMER → HTX_MEMBER via join approval)
                        return existing;
                }
                ...
                try {
                        ...
                        log.warn("   ⚠️ Không thể tạo {}: {} (Có thể SĐT hoặc Email đã tồn tại ở User khác)", username,
                                        e.getMessage());
                        return null;
                }
        }
        ...
        private Shop ensureShop(User owner, String slug, String name, String province,
                        String ward, String bio, short yearsExp, int farmAreaM2) {
                if (owner == null) {
                ...
                if (shopRepository.existsByOwner_Id(owner.getId())) {
                        ...
                        return shopRepository.findFirstByOwner_Id(owner.getId()).orElse(null);
                }
                ...
                try {
                        Shop shop = Shop.builder()
                                        ...
                                        .build();
                        shopRepository.save(shop);
                        log.info("   ✅ Tạo shop: {} — {}, {}", name, province, ward);
                        return shop;
                } catch (Exception e) {
